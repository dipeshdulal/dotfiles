...
Before issuing any control command, check whether the `herdr` CLI is available:

```bash
herdr --help
```

If the check fails (binary not found, server not running, or no active session), report the error and stop. Do not attempt to control Herdr when the CLI is unavailable.

When the CLI is available, the `herdr` binary in `PATH` talks to the current session (or to explicitly targeted workspaces/panes). Use it to inspect neighboring work, create terminal layout, start agents and commands, read output, and wait for state changes.

When running inside a Herdr-managed pane (`HERDR_ENV=1`), the CLI talks to the current session automatically via injected context (`$HERDR_WORKSPACE_ID`, `$HERDR_TAB_ID`, `$HERDR_PANE_ID`). When running from outside Herdr (e.g., from an agent gateway), target workspaces, tabs, and panes by their explicit IDs rather than relying on environment variables.
